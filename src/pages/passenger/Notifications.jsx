...
const Notifications = () => {
  ...
  // ==========================================================
  // METRICS & STATISTICS (Dynamic calculations)
  // ==========================================================

  const stats = useMemo(() => {
    const total = notifications.length;
    const unread = unreadCount;
    const bookings = notifications.filter(n => 
      n.type === 'booking' || (n.category && n.category.toLowerCase() === 'booking')
    ).length;
    const smartSeat = notifications.filter(n => 
      n.type === 'smartseat' || n.type === 'adjacent_seat' || (n.category && n.category.toLowerCase() === 'smartseat')
    ).length;

    return { total, unread, bookings, smartSeat };
  ...
  const getTypeConfig = (type, category) => {
    ...
    if (lowerType.includes('smartseat') || lowerType.includes('adjacent')) {
      return {
        icon: Compass,
    ...
    if (lowerType.includes('delay') || lowerType.includes('travel')) {
      return {
        icon: Clock,
  ...
  const renderNotificationCard = (notification) => {
    ...
    const gender = metadata.gender || notification.passengerGender;
    const age = metadata.age || notification.passengerAge;
    const passengerName = metadata.passengerName || notification.passengerName;

    return (
      <div 
        key={notifId} 
        className={`notification-card-3d ${config.railClass} ${isUnread ? 'notification-card-unread' : 'opacity-90'}`}
      >
        <div className="flex items-start gap-4">
          ...
          {/* ICON BADGE */}
          <div className={`p-3 rounded-2xl border ${config.badgeBg} flex items-center justify-center flex-shrink-0 shadow-lg`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center justify-between gap-2 mb-1">
              <div className="flex items-center gap-2">
                <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded border ${config.badgeBg}`}>
                  {config.label}
                </span>
                {isUnread && (
              ...
              <span className="text-xs text-slate-400 font-mono">{formatTime(notification.createdAt)}</span>
            </div>

            <h4 className="font-bold text-white text-base mt-1">{notification.title}</h4>
            <p className="text-xs text-slate-300 mt-1 leading-relaxed">{notification.message}</p>

            {/* EMBEDDED SMART TRAVEL PASS TICKET (if booking info exists) */}
            {(notification.seatNumber || metadata.from || metadata.to) && (
              <div className="smart-ticket-card mt-3 text-xs space-y-2">
                <div className="flex items-center justify-between border-b border-slate-800 pb-1.5 text-cyan-300 font-mono text-[10px] font-bold uppercase">
                  <span>SMARTSEAT PASS TELEMETRY</span>
            ...
            {(passengerName || gender || age) ? (
              <div className="mt-3 p-2.5 bg-slate-900/90 rounded-xl border border-slate-800 flex items-center justify-between text-xs text-slate-300">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-white">{passengerName || 'Passenger'}</span>
                </div>
                <div className="flex items-center gap-3 text-[11px] text-slate-400">
                  {age && <span>Age: {age}</span>}
                  {gender && <span className="capitalize">Gender: {gender}</span>}
                </div>
              </div>
            ) : notification.type === 'booking' && (
              <div className="mt-2 text-[10px] text-slate-500 italic flex items-center gap-1">
                <ShieldAlert className="w-3 h-3 text-slate-500" />
                <span>Passenger details protected by privacy rules</span>
              </div>
            )}
          </div>
      ...
      <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-black tracking-widest text-cyan-400 uppercase">SMARTSEAT INTELLIGENCE</span>
              </div>
              ...
              <p className="text-sm text-slate-300 mt-2 max-w-xl">
                Real-time journey telemetry, booking confirmations, SmartSeat monitoring alerts, and system updates.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              <div className="px-4 py-3 bg-slate-900/90 rounded-2xl border border-slate-800 text-center">
                <div className="flex items-center gap-2 text-cyan-400 justify-center">
                  ...
                  <span className="text-2xl font-black">{stats.unread}</span>
                </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="stat-card">
            <div className="flex items-center justify-between text-cyan-400">
              ...
              <span className="text-[10px] font-bold text-cyan-400 bg-cyan-950/80 px-2 py-0.5 rounded border border-cyan-800">ACTIVE</span>
            </div>
            <p className="text-2xl font-black text-white mt-2">{stats.unread}</p>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">UNREAD ALERTS</p>
          </div>

          <div className="stat-card">
            <div className="flex items-center justify-between text-emerald-400">
              <Ticket className="w-5 h-5" />
              <span className="text-[10px] font-bold text-emerald-400 bg-emerald-950/80 px-2 py-0.5 rounded border border-emerald-800">PASSED</span>
            </div>
            <p className="text-2xl font-black text-white mt-2">{stats.bookings}</p>
          ...
          <div className="stat-card">
            <div className="flex items-center justify-between text-sky-400">
              ...
              <span className="text-[10px] font-bold text-sky-400 bg-sky-950/80 px-2 py-0.5 rounded border border-sky-800">LIVE</span>
            </div>
            <p className="text-2xl font-black text-white mt-2">{stats.smartSeat}</p>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">SMARTSEAT</p>
          </div>

          <div className="stat-card">
            <div className="flex items-center justify-between text-slate-400">
              <ShieldCheck className="w-5 h-5" />
              <span className="text-[10px] font-bold text-slate-400 bg-slate-900 px-2 py-0.5 rounded border border-slate-800">TOTAL</span>
            </div>
            <p className="text-2xl font-black text-white mt-2">{stats.total}</p>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">TOTAL TELEMETRY</p>
          </div>
        </div>
        ...
        {loading ? (
          <div className="space-y-4 py-6">
            {[1, 2, 3].map(i => (
              <div key={i} className="h-28 rounded-2xl skeleton-shimmer border border-slate-800" />
            ))}
          </div>
          ...
          <div className="bg-slate-900/60 rounded-3xl p-12 border border-slate-800 text-center">
            ...
            <h3 className="text-xl font-bold text-white uppercase tracking-wide">SYSTEM CLEAR</h3>
          ...
          <div className="space-y-8 relative">
            ...
            {Object.entries(timelineGroups).map(([groupName, items]) => {
              ...
              return (
                <div key={groupName} className="space-y-4 relative pl-8 sm:pl-14">
                  {/* GROUP HEADER */}
                  <div className="flex items-center gap-3">
                    <div className="timeline-node" style={{ backgroundColor: groupName === 'TODAY' ? '#38bdf8' : '#64748b' }} />
