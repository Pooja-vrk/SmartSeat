...
const CancelDialog = ({
  ...
  const boardingPointName = (typeof booking?.boardingPoint === 'object' ? booking.boardingPoint?.name : booking?.boardingPoint) || route.source || 'N/A';
  const droppingPointName = (typeof booking?.droppingPoint === 'object' ? booking.droppingPoint?.name : booking?.droppingPoint) || route.destination || 'N/A';
          ...
          <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 space-y-2 text-sm">
            ...
            {(booking?.boardingPoint || booking?.droppingPoint) && (
              <div className="text-xs text-slate-600">
                <span>Stops: <strong>{boardingPointName}</strong> → <strong>{droppingPointName}</strong></span>
              </div>
            )}
...
const MyBookings = () => {
  ...
  const openCancelDialog = async (booking) => {
    ...
    try {
      const preview = await bookingService.getRefundPreview(
        booking._id
      );
          ...
          <div className="p-6 sm:p-8 rounded-3xl bg-slate-900/95 text-white border border-slate-800 shadow-xl flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-400">
                View, manage and cancel your travel bookings
              </p>
            ...
            <Link to="/search">
              ...
              <Button
                variant="primary"
                className="bg-gradient-to-r from-cyan-500 to-teal-500 text-slate-950 font-black px-5 py-2.5 rounded-xl text-xs uppercase tracking-wider shadow-lg shadow-cyan-500/20"
              >
                ✦ Book New Bus
              </Button>
          ...
          {bookings.length === 0 ? (
              ...
              {activeTab === 'upcoming' && (
                ...
                <Link to="/search">
                  ...
                  <Button
                    variant="primary"
                    icon={Bus}
                    className="bg-cyan-500 text-slate-950 font-bold text-xs uppercase"
                  >
                    Search Buses
                  </Button>
            ...
            <div className="space-y-4">
              ...
              {bookings.map((booking) => (
                ...
                <div
                  key={booking._id}
                  className="booking-card-premium p-6"
                >
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
                    ...
                    <div className="flex-1 space-y-4">
                      ...
                      <div className="flex items-start justify-between border-b border-slate-100 pb-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-3 flex-wrap">
                            ...
                            {/* IMPORTANT:
                                Completed tab displays COMPLETED
                                even if backend bookingStatus remains confirmed.
                            */}
                            {getStatusBadge(
                              activeTab === 'completed'
                                ? 'completed'
                          ...
                          {(() => {
                            const bPoint = (typeof booking.boardingPoint === 'object' ? booking.boardingPoint?.name : booking.boardingPoint) || booking.routeId?.source;
                            const dPoint = (typeof booking.droppingPoint === 'object' ? booking.droppingPoint?.name : booking.droppingPoint) || booking.routeId?.destination;
                            const bTime = typeof booking.boardingPoint === 'object' ? booking.boardingPoint?.time : null;
                            const dTime = typeof booking.droppingPoint === 'object' ? booking.droppingPoint?.time : null;
                            ...
                            return (
                              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-600 mt-2 pt-2 border-t border-slate-100">
                                <span className="inline-flex items-center gap-1">
                                  ...
                                  <span>Board: <strong className="text-slate-800">{bPoint || 'N/A'}</strong>{bTime ? ` (${bTime})` : ''}</span>
                                </span>
                                <span className="text-slate-300">→</span>
                                <span className="inline-flex items-center gap-1">
                                  <MapPin className="w-3 h-3 text-rose-500" />
                                  <span>Drop: <strong className="text-slate-800">{dPoint || 'N/A'}</strong>{dTime ? ` (${dTime})` : ''}</span>
                                </span>
                              </div>
                            );
                          })()}
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                        ...
                        <div>
                          ...
                          <p className="font-bold text-xs text-cyan-700 font-mono">
                            {booking.seatNumber || 'N/A'}
                          </p>
                    ...
                    <div className="flex lg:flex-col gap-2.5 border-t lg:border-t-0 lg:border-l border-slate-100 pt-4 lg:pt-0 lg:pl-6">
                      ...
                      {/* VIEW TICKET
                          Available for all booking tabs */}
                      ...
                      {activeTab === 'upcoming' &&
                        booking.bookingStatus === 'confirmed' && (
                          ...
                          <>
                            ...
                            {/* CHANGE SEAT */}
                            <Link
                              to={`/change-seat/${booking._id}`}
                            >

                              <Button
                                variant="outline"
                                size="sm"
                                className="w-full text-xs font-bold uppercase tracking-wider text-cyan-700 border-cyan-300"
                              >
                                Change Seat
                              </Button>
